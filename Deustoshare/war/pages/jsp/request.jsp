	...
	<div class="container">
		...
		<form id="newRequestForm" method="post" action="#">
			<div class="new_resource_form">
				<ul>
					<li><span class="first_field">Resource: <input
							...
							disabled value="<%=resource.getTitle()%>"/></span> <span class="second_field">User: <input
							type="text" class="text" name="user" placeholder="User" disabled value="<%=resource.getOwner().getName()%>" /></span>
					</li>
			...
			<div class="12u row-button">
				<a href="javascript:setContentForm('start/request/create?resourceid=<%=resource.getKey().getId()%>&ownerEmail=<%=resource.getOwner().getEmail()%>','newRequestForm')" class="button submit">Make Request</a>
			</div>
		</form>
