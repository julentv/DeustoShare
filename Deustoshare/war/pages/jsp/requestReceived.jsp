	...
	<div class="container">
		...
		<form id="newRequestForm" method="post" action="#">
			...
			<div class="12u row-button">
				<a
					href="javascript:setContent('start/received_requests/changeState?requestid=<%=requestVar.getKey().getId()%>&requesterEmail=<%=requestVar.getKey().getParent().getName()%>&newState=Accepted')"
					class="button submit">Accept</a>
				<a
					href="javascript:setContent('start/received_requests/changeState?requestid=<%=requestVar.getKey().getId()%>&requesterEmail=<%=requestVar.getKey().getParent().getName()%>&newState=Rejected')"
					class="buttonRed submit">Reject</a>
			</div>
		</form>
